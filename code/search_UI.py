...
class search_UI(QtWidgets.QDialog, search_GUI.Ui_Dialog):
    ...
    def prepare(self):
        ...
        if self.word.lower() != keyword:
            self.word = keyword
            self.index = -1
        self.pos = [(i, i + len(keyword)) for i in positions(self.ob.textEdit.toPlainText().lower(), keyword)]

    def next_word(self):
        self.ob.textEdit.setText(self.ob.textEdit.toPlainText())
        self.prepare()
        keyword = self.lineEdit.text().lower()
        if keyword:
            try:
